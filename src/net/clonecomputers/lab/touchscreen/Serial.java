...
*/
  public static boolean touchPort(String iname, int irate) throws SerialException {
	...
	Enumeration<CommPortIdentifier> portList = CommPortIdentifier.getPortIdentifiers();
      ...
      throw new SerialException(
        String.format("Serial port ''%s'' already in use. Try quitting any programs that may be using it.", iname)
      );
    } catch (Exception e) {
      throw new SerialException(
        String.format("Error touching serial port ''%s''.", iname), e
      );
    }
	return result;
  }
  ...
  throws SerialException {
	...
	Enumeration<CommPortIdentifier> portList = CommPortIdentifier.getPortIdentifiers();
      ...
      throw new SerialException(
        String.format(
          "Serial port ''%s'' already in use. Try quiting any programs that may be using it.",
          iname
        )
      );
    } catch (Exception e) {
  ...
  public int readBytesUntil(int interesting, byte outgoing[]) {
    ...
    synchronized (buffer) {
      ...
      if (length > outgoing.length) {
        System.err.println(
          String.format(
            "readBytesUntil() byte buffer is too small for the %d" +
              " bytes up to and including char %d",
            length,
            interesting
          )
        );
